...
class DataAccess
{
	...
	function login($emailL, $LPass)
	{
		$query = "select * from utilizador where email = '$emailL' and senha = '$LPass'";
	 	$this->connect();
	...
	{
		$query = "update utilizador set Nome='$novoNome',senha='$novaPwd' where id = '$id'";
		$this->connect();
		$this->execute($query);
		$this->disconnect();
	}
